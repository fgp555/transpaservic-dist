...
let OrderSave2Service = class OrderSave2Service {
    constructor(orderRepository, operatorRepository, notificationService, waSenderService) {
    ...
    async create(createOrderDto) {
        ...
        try {
            ...
            if (createOrderDto.sendWhatsApp) {
                try {
                    const wa_resp = await this.waSenderService.orden_emitida(orderPayload);
    ...
    async saveArrayData(dataArray, sendToWhatsApp) {
        ...
        if (sendToWhatsApp) {
            ...
            for (const [phone, info] of phoneMap.entries()) {
                ...
                try {
                    await this.waSenderService.ordenes_emitidas(ordersPayload);
