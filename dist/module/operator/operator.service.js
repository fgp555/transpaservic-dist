...
let OperatorService = class OperatorService {
    ...
    async create(createOperatorDto) {
        const { name, username, whatsapp, email } = createOperatorDto;
        const existingName = await this.operatorRepository.findOne({
            where: { name },
        });
        if (existingName) {
            throw new common_1.ConflictException(`El nombre "${name}" ya está en uso.`);
        }
        const existingUsername = await this.operatorRepository.findOne({
            where: { username },
        });
        if (existingUsername) {
            throw new common_1.ConflictException(`El nombre de usuario "${username}" ya está en uso.`);
        }
        const existingWhatsapp = await this.operatorRepository.findOne({
            where: { whatsapp },
        });
        if (existingWhatsapp) {
            throw new common_1.ConflictException(`El número de WhatsApp "${whatsapp}" ya está en uso.`);
        }
        const existingEmail = await this.operatorRepository.findOne({
            where: { email },
        });
        if (existingEmail) {
    ...
    async findAll(filters) {
        ...
        (
          ...
          operator.username LIKE :search OR
    ...
    async update(id, updateOperatorDto) {
        await this.operatorRepository.update(id, updateOperatorDto);
        return this.findOne(id);
    }
    async remove(id) {
        const operator = await this.operatorRepository.findOne({ where: { id } });
        if (!operator) {
            throw new common_1.HttpException('El operatore no fue encontrado', common_1.HttpStatus.NOT_FOUND);
        }
        const relatedOrders = await this.orderRepository.find({
            where: { operator: { id } },
        });
        if (relatedOrders.length > 0) {
            throw new common_1.HttpException(`No se puede eliminar el operatore con ID ${id} porque tiene orders asociados.`, common_1.HttpStatus.CONFLICT);
        }
        await this.operatorRepository.delete(id);
        return { message: `El operatore con ID ${id} fue eliminado exitosamente` };
    }
};
