"use strict";
var __decorate = (this && this.__decorate) || function (decorators, target, key, desc) {
    var c = arguments.length, r = c < 3 ? target : desc === null ? desc = Object.getOwnPropertyDescriptor(target, key) : desc, d;
    if (typeof Reflect === "object" && typeof Reflect.decorate === "function") r = Reflect.decorate(decorators, target, key, desc);
    else for (var i = decorators.length - 1; i >= 0; i--) if (d = decorators[i]) r = (c < 3 ? d(r) : c > 3 ? d(target, key, r) : d(target, key)) || r;
    return c > 3 && r && Object.defineProperty(target, key, r), r;
};
var __metadata = (this && this.__metadata) || function (k, v) {
    if (typeof Reflect === "object" && typeof Reflect.metadata === "function") return Reflect.metadata(k, v);
};
var __param = (this && this.__param) || function (paramIndex, decorator) {
    return function (target, key) { decorator(target, key, paramIndex); }
};
Object.defineProperty(exports, "__esModule", { value: true });
exports.SqlController = void 0;
const common_1 = require("@nestjs/common");
const sql_service_1 = require("./sql.service");
let SqlController = class SqlController {
    constructor(sqlService) {
        this.sqlService = sqlService;
    }
    async runSql(query) {
        const result = await this.sqlService.executeRawQuery(query);
        return { result };
    }
};
exports.SqlController = SqlController;
__decorate([
    (0, common_1.Post)('run'),
    __param(0, (0, common_1.Body)('query')),
    __metadata("design:type", Function),
    __metadata("design:paramtypes", [String]),
    __metadata("design:returntype", Promise)
], SqlController.prototype, "runSql", null);
exports.SqlController = SqlController = __decorate([
    (0, common_1.Controller)('sql'),
    __metadata("design:paramtypes", [sql_service_1.SqlService])
], SqlController);
//# sourceMappingURL=sql.controller.js.map